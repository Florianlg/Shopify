...
document.addEventListener("DOMContentLoaded", () => {
  ...
  function updatePromoMessage(cart) {
    ...
    if (total < 50) {
      promoContainer.textContent = `Plus que ${(50 - total).toFixed(
        2
      )} € pour la livraison gratuite.`;
    } else if (total < 100) {
      promoContainer.textContent = `Plus que ${(100 - total).toFixed(
        2
      )} € pour recevoir un cadeau offert.`;
    } else {
  ...
  function lockGift() {
    const giftInput = document.querySelector(
      `input[data-quantity-variant-id="${GIFT_VARIANT_ID}"]`
    );
    if (!giftInput) return;
